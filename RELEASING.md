# Releasing Ashford Events

Repo layout assumption: the repository root **is** the plugin folder
(`ashford-events.php` at the root). The workflow lives at
...
2. In `ashford-events.php`, set the repo URL in the update-checker block
   (`https://github.com/YOUR-ORG/YOUR-REPO/`). Commit.
3. **Private repo only:** on each site (or in a mu-plugin shared across your
   maintenance stack), add to `wp-config.php`:

   ```php
   define( 'ASH_EVENTS_GITHUB_TOKEN', 'github_pat_XXXX' );
   ```
...
2. Bump the version in **two places** (they must match the tag):
   - `ashford-events.php` → `Version:` header **and** the
     `ASH_EVENTS_VERSION` constant
   - `readme.txt` → `Stable tag:`
3. Commit, then tag and push:

   ```bash
   git commit -am "Fix popover clipping on last row"
   git tag v1.3.1
   git push && git push --tags
   ```

4. The Action builds `ashford-events.zip` and attaches it to a GitHub
...
- Patch (`1.3.x`) — fixes, style tweaks
- Minor (`1.x.0`) — new features (views, importer capabilities)
